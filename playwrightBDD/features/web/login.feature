...
@mock
Scenario: Verify login error when API returns mocked failure
  Given the login API is mocked to return "Service unavailable"
  And user is in login page
  And User enter the username as "testuser" and password as "Test@123"
  When user click login button
  Then user should see login "Service unavailable"
